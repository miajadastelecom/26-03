...
function ticketToFirestore(t) { return { ...t, imagenes: JSON.stringify(t.imagenes || []), comentarios: JSON.stringify(t.comentarios || []) }; } 
function ticketFromFirestore(t) { return { ...t, imagenes: typeof t.imagenes === "string" ? JSON.parse(t.imagenes) : (t.imagenes || []), comentarios: typeof t.comentarios === "string" ? JSON.parse(t.comentarios) : (t.comentarios || []) }; }
...
// ─── COMPONENTES ─────────────────────────────────────────────────────────────
function Badge({ texto, color, small }) { return ( <span style={{ background: color + "22", color, border: `1px solid ${color}55`, borderRadius: 4, padding: small ? "1px 7px" : "3px 10px", fontSize: small ? 10 : 11, fontWeight: 700 }}> {texto} </span> ); }
...
function ModalCrearTicket({ usuarioActual, onClose, onCrear }) {
  ...
  const submit = () => {
    ...
    onCrear({
      ...
      estado: "Pendiente",
      vistoPorTrabajador: false, // <-- [1] Primer Ticket (Enviado)
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#00000099", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <div style={{ background: "#111827", padding: 28, borderRadius: 14, width: "100%", maxWidth: 500 }}>
        <h2 style={{ color: "#fff" }}>Nuevo Ticket</h2>
        <input style={inp} placeholder="Título" value={titulo} onChange={e => setTitulo(e.target.value)} />
        <div style={{ marginTop: 20 }}>
          {EMPRESAS.filter(e => e.id !== 0).map(e => (
            <button key={e.id} onClick={() => setEmps([e.id])} style={{ ...btnS, background: empresasDestino.includes(e.id) ? e.color : "#1A2235", color: "#fff", marginRight: 5 }}>{e.nombre}</button>
          ))}
        </div>
        <div style={{ marginTop: 20, display: "flex", justifyContent: "flex-end" }}>
          <button onClick={submit} style={{ ...btnS, background: "#3182CE", color: "#fff" }}>Crear</button>
        </div>
      </div>
    </div>
  );
...
function ModalDetalle({ ticket, usuarioActual, onClose, onActualizar }) {
  ...
  // <-- [5] Dos Ticks Verdes cuando el trabajador visualiza
  useEffect(() => {
    if (esAsignado && !ticket.vistoPorTrabajador) {
      onActualizar({ ...ticket, vistoPorTrabajador: true });
    }
  }, [ticket.id, esAsignado, ticket.vistoPorTrabajador, onActualizar]);

  const asignar = () => {
    const nuevas = { ...ticket.asignacionesPorEmpresa, [usuarioActual.empresaId]: seleccionados };
    // <-- [6] Dos Ticks cuando el encargado asigna (el estado cambia de Pendiente)
    ...
    <div style={{ position: "fixed", inset: 0, background: "#00000099", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <div style={{ background: "#111827", padding: 28, borderRadius: 14, width: "100%", maxWidth: 600 }}>
        ...
        {usuarioActual.rol === "encargado" && (
          <div>
            <h4 style={{ color: "#fff" }}>Asignar personal</h4>
            {USUARIOS.filter(u => u.empresaId === usuarioActual.empresaId && u.rol === "trabajador").map(u => (
              <label key={u.id} style={{ display: "block", color: "#fff" }}>
...
function TarjetaTicket({ ticket, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ background: "#111827", border: "1px solid #1E293B", borderRadius: 10, padding: "16px 18px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      {/* LÓGICA DE TICKS [Sin tocar diseño base] */}
      <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
        {ticket.vistoPorTrabajador ? (
          <span style={{ color: "#38A169", fontWeight: "bold", fontSize: 16 }}>✓✓</span>
...
export default function App() {
  ...
  if (!logueado) {
    return (
      <div style={{ minHeight: "100vh", background: "#0A0F1C", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <button onClick={() => { setLogueado(true); setUsuarioId(1); }} style={{ ...btnS, background: "#3182CE", color: "#fff", padding: "10px 20px" }}>Login Encargado</button>
        <button onClick={() => { setLogueado(true); setUsuarioId(2); }} style={{ ...btnS, background: "#38A169", color: "#fff", padding: "10px 20px", marginLeft: 10 }}>Login Trabajador</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1C", padding: 20 }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
          <h1 style={{ color: "#fff" }}>Tickets</h1>
          <button onClick={() => setModalCrear(true)} style={{ ...btnS, background: "#3182CE", color: "#fff" }}>+ Nuevo</button>
        </header>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {tickets.map(t => <TarjetaTicket key={t.id} ticket={t} onClick={() => setDetalle(t)} />)}
        </div>
      </div>
